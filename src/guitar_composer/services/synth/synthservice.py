...
from guitar_composer.services.synth.sequencer import sequencer
import atexit
import signal
...
class synthservice:
    ...
    def shutdown(self):
        ...
        # stop synth thread
        self.stop()
        # failsafe in case there is some race condition during shutdown
        signal.alarm(2)
    ...
    def start(self):
        # start audio threads, load sound fonts, generates a file called instruments.json
        # which contains meta data for the various intruments.
        gcsynth.start({"sfpaths": self.db.sfpaths})

        # read instrument data generated by gcsynth
        self.db.setup()
    ...
    def instrument_groups(self):
        return self.db.instrumentGroups()
